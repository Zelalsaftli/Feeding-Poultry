...
import { CATEGORY_NAMES_EN } from '../constants';
...
const SelectionPage: React.FC<SelectionPageProps> = ({ 
  ...
  const handleRequestResetIngredients = () => {
    setConfirmation({
      isOpen: true,
      title: 'Restore Default Ingredients',
      message: 'Are you sure you want to restore the default ingredient list? All custom modifications will be lost.',
  ...
  const handleRequestResetEnzymes = () => {
    setConfirmation({
      isOpen: true,
      title: 'Restore Default Enzymes',
      message: 'Are you sure you want to restore the default enzyme list? All custom modifications will be lost.',
      ...
      reader.onload = (e) => {
          ...
          if (text) {
              ...
              if (importedIngredients.length > 0) {
                  if (window.confirm(`Found ${importedIngredients.length} ingredients. This will update existing ingredients and add new ones to the master list. Do you want to proceed?`)) {
        ...
        <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200">
          <div className="flex flex-wrap justify-between items-center mb-2 gap-4">
             ...
             <div className="flex gap-2 flex-wrap">
                ...
                <button
                    ...
                    className="bg-gray-200 text-gray-700 font-semibold py-2 px-4 rounded-md hover:bg-gray-300 transition-colors text-sm"
                >
                    Restore Default Ingredients
                </button>
                 <button
                    onClick={handleRequestResetEnzymes}
                    className="bg-gray-200 text-gray-700 font-semibold py-2 px-4 rounded-md hover:bg-gray-300 transition-colors text-sm"
                 >
                    Restore Default Enzymes
                 </button>
             </div>
          </div>
          <p className="text-gray-600 mb-6">Select the ingredients and enzymes you want to include in your feed. You can edit the default analysis of any item by clicking its name.</p>
          ...
          <div className="mb-6">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {(Object.keys(CATEGORY_NAMES_EN) as IngredientCategory[]).map(categoryKey => (
                <button
                  key={categoryKey}
          ...
          <div className="space-y-4">
            {displayedCategories.map(category => (
              <div key={category} className="border border-gray-200 rounded-lg overflow-hidden transition-all duration-300">
                ...
                >
                  <h3 className="text-xl font-semibold text-teal-700">{CATEGORY_NAMES_EN[category]}</h3>
            ...
            <div key="enzymes" className="border border-gray-200 rounded-lg overflow-hidden transition-all duration-300">
               <button
                ...
                >
                    <h3 className="text-xl font-semibold text-teal-700">Enzyme Supplements</h3>
                    <div className="flex items-center">
                        <div className="flex space-x-2 text-sm ml-4">
                            <span onClick={(e) => { e.stopPropagation(); handleSelectAllEnzymes(true); }} className="font-medium text-teal-600 hover:text-teal-800 cursor-pointer p-1">Select All</span>
                            <span onClick={(e) => { e.stopPropagation(); handleSelectAllEnzymes(false); }} className="font-medium text-gray-500 hover:text-gray-700 cursor-pointer p-1">Deselect All</span>
                        </div>
                            ...
                            {masterEnzymes.map(enzyme => (
                                <div key={enzyme.id} className="flex items-center group justify-between">
                                    <div className="flex items-center">
                                        ...
                                        />
                                        <label htmlFor={`enzyme-${enzyme.id}`} className="ml-3 text-sm font-medium text-gray-700 select-none cursor-pointer">
                                            {enzyme.name}
                                        </label>
                                        <div className="relative flex items-center group/tooltip ml-2">
                                            ...
                                            <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 w-max max-w-xs p-3 bg-gray-800 text-white text-xs rounded-md shadow-lg opacity-0 group-hover/tooltip:opacity-100 transition-opacity pointer-events-none z-10">
                                                <p className="font-semibold">Standard Dosage: <span className="font-mono">{enzyme.standard_dosage_g_per_ton}</span> g/ton</p>
                                                <p className="font-semibold">Price: <span className="font-mono">${enzyme.Price_USD_per_ton}</span> / ton</p>
        ...
        <div className="bg-white p-4 rounded-lg shadow-md border border-gray-200 sticky bottom-4">
          <div className="flex justify-between items-center">
            <p className="font-semibold text-gray-700">
              ...
              and <span className="text-teal-600 text-lg">{selectedEnzymeIds.size}</span> enzymes
            </p>
            <button
