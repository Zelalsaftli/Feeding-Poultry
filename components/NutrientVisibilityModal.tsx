...
import { ANALYSIS_RESULTS_EN } from '../constants';
...
const MODAL_NUTRIENT_GROUPS: Record<string, string[]> = {
  ...
  'Minerals & Electrolytes': ['nutrients.Ca_pct', 'nutrients.avP_pct', 'nutrients.Na_pct', 'nutrients.K_pct', 'nutrients.Cl_pct', 'nutrients.Ash_pct', 'nutrients.CaAvP_Ratio', 'nutrients.K_Cl_Na_Ratio', 'nutrients.dEB'],
...
const NutrientVisibilityModal: React.FC<NutrientVisibilityModalProps> = ({ isOpen, onClose, onSave, currentVisibility }) => {
  ...
  const handleSelectAll = (select: boolean) => {
    ...
    Object.keys(ANALYSIS_RESULTS_EN).forEach(key => {
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl p-6 m-4 max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex-grow overflow-y-auto pl-2">
            <div className="flex justify-between items-center mb-4 sticky top-0 bg-white py-2 z-10">
                 <p className="text-sm text-gray-600">Choose the items you want to display in the report.</p>
                        ...
                        {keys.map(key => (
                           ANALYSIS_RESULTS_EN[key] && (
                             <label key={key} className="flex items-center space-x-2 cursor-pointer p-1 rounded-md hover:bg-gray-100">
                                ...
                                />
                                <span className="text-sm text-gray-800 select-none">{ANALYSIS_RESULTS_EN[key]}</span>
                            </label>
                           )
                        ))}
                    </div>
